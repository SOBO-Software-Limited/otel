...
void Metrics::InitPrometheus(const std::string &addr) { // To be Deprecated in Otel API
...
Metrics::AddCounterSumView(const std::string &name, const std::string &description) {
    ...
    auto p = std::static_pointer_cast<metrics_sdk::MeterProvider>(metrics_api::Provider::GetMeterProvider());
    std::shared_ptr<opentelemetry::metrics::Meter> meter = p->GetMeter("zilliqa", "1.2.0");
    // counter view
...
Metrics::AddCounterHistogramView(const std::string name, std::list<double> list,
    ...
    std::unique_ptr<metrics_sdk::InstrumentSelector> histogram_instrument_selector{
            new metrics_sdk::InstrumentSelector(metrics_sdk::InstrumentType::kHistogram, name)};

    std::unique_ptr<metrics_sdk::MeterSelector> histogram_meter_selector{
            new metrics_sdk::MeterSelector(ZILLIQA_METRIC_FAMILY, METRIC_ZILLIQA_SCHEMA_VERSION,
    ...
    std::unique_ptr<metrics_sdk::View> histogram_view{
            new metrics_sdk::View{name, description, metrics_sdk::AggregationType::kHistogram, aggregation_config,}};
...
Metrics::GetMeter() {
    const auto p = std::static_pointer_cast<metrics_sdk::MeterProvider>(metrics_api::Provider::GetMeterProvider());
    return p->GetMeter(ZILLIQA_METRIC_FAMILY, METRIC_ZILLIQA_SCHEMA_VERSION, METRIC_ZILLIQA_SCHEMA);
}
